...
//        self.drawNeedle(progressValue: 0.5)
    ...
    func drawArrowIcon() {
        ...
        let image = UIImage(named: "dashboard_pointer_blue")
        arrowIcon = UIImageView(image: image)
        
        // x 需要 减去箭头宽度的一半
        // y 需要 减去箭头高度的一半
        arrowIcon?.frame = CGRect(x: view.center.x - 16, y: view.center.y - 6, width: 31, height: 12)
        view.addSubview(arrowIcon!)
        
        // 创建了一条横线， 方便查看 arrowIcon 围绕哪个点进行旋转
        let label = UILabel(frame: CGRect(x: 0, y: view.center.y, width: 400, height: 2))
        label.backgroundColor = UIColor.cyan
        view.addSubview(label)
        
        // 创建了一条竖线， 方便查看 arrowIcon 围绕哪个点进行旋转
        let label1 = UILabel(frame: CGRect(x: view.center.x, y: 0, width: 2, height: 600))
        label1.backgroundColor = UIColor.cyan
        view.addSubview(label1)
        
        // 设置imageview的边框颜色，需要删除
        arrowIcon?.layer.borderWidth = 1
        arrowIcon?.layer.borderColor = UIColor.red.cgColor
        
        // anchorPoint 锚点 锚点为自身中心 （即围绕哪个点旋转）
        // 解释 (6/31 === (箭头中小球宽度的一半) / 箭头的宽度)
        arrowIcon?.layer.anchorPoint = CGPoint(x: 1 - (6/31), y: 0.5)
    ...
    @IBAction func sliderAction(_ sender: UISlider) {
        ...
        // 动态更改旋转的角度
        arrowIcon?.transform = CGAffineTransform(rotationAngle: -Double.pi * CGFloat(sender.value))
    }
